...
class TestSettingsSeparatorMixed(TestCase):
    def test_assign_duplicate_list(self):
        ...
        p = NestedParser(
            data,
            {"raise_duplicate": False, "assign_duplicate": True, "separator": "mixed"},
        )
    ...
    def test_mixed_list_empty_index(self):
        data = {
            "title": "lalal",
            "article[]": "lalal",
        }
    ...
    def test_mixed_empty_obj_2(self):
        data = {
            "title": "lalal",
            "article[0].op": "lalal",
        }
        parser = NestedParser(data, {"separator": "mixed"})
        self.assertTrue(parser.is_valid())
        expected = {"title": "lalal", "article": [{"op": "lalal"}]}
        self.assertDictEqual(parser.validate_data, expected)
    ...
    def test_mixed_empty_object_dot2(self):
        data = {
            "title": "lalal",
        ...
        parser = NestedParser(data, {"separator": "mixed"})
        self.assertTrue(parser.is_valid())
        expected = {"title": "lalal", "article": [{"op": [{"e": {}}]}]}
        self.assertDictEqual(parser.validate_data, expected)
