...
class TestParser(TestCase):
    ...
    def test_parser_empty_object(self):
        data = {"title[id][]": "lalal"}
        parser = NestedParser(data, {"separator": "bracket"})
        self.assertTrue(parser.is_valid())
        expected = {"title": {"id": []}}
    ...
    def test_wrong_settings(self):
        ...
        with self.assertRaises(AssertionError):
            NestedParser(data, options={"separator": "worng"})
        with self.assertRaises(AssertionError):
            NestedParser(data, options={"raise_duplicate": "need_boolean"})
        with self.assertRaises(AssertionError):
            NestedParser(data, options={"assign_duplicate": "need_boolean"})
